...
function collectionSessionDetail(sessionId) {
  ...
  const kebabHead =
    session.status === "open"
      ? `<div class="kebab-wrap"><button class="kebab-btn">⋮</button><div class="kebab-menu"><button data-action="attach-all-session" data-attach-session="${session.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:8px;flex-shrink:0" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>Add all from this day</button><button class="danger" data-action="delete-session" data-session-delete="${session.id}" data-session-date="${session.date}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:8px;flex-shrink:0" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>Delete session</button></div></div>`
      : "";
  modal(
...
function renderCollectionSessions() {
  ...
  const current = session
    ? `<section class="panel glass session-hero"><div class="panel-header"><div><h3>Today's session</h3><span class="muted">${dateOnly(dateFromKey(today))}</span></div><div class="header-actions"><span class="badge ${session.status === "open" ? "paid" : "active"}">${session.status === "open" ? "Open" : "Closed"}</span><div class="kebab-wrap"><button class="kebab-btn">⋮ More</button><div class="kebab-menu"><button data-action="attach-all-session" data-attach-session="${session.id}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:8px;flex-shrink:0" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>Add all from this day</button><button class="danger" data-action="delete-session" data-session-delete="${session.id}" data-session-date="${session.date}"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:8px;flex-shrink:0" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>Delete session</button></div></div></div></div><div class="session-summary"><div><span>Collected</span><strong>${money(sessionTotal(session))}</strong></div><div><span>Payments</span><strong>${(session.paymentIds || []).length}</strong></div><div><span>Started</span><strong>${dateTime(session.startedAt)}</strong></div>${session.status === "closed" ? `<div><span>Cash counted</span><strong>${money(session.cashCounted)}</strong></div><div><span>Difference</span><strong class="${Math.abs(Number(session.difference || 0)) < 0.001 ? "good-text" : "warn-text"}">${money(session.difference)}</strong></div>` : ""}</div>${session.status === "open" ? `<div class="session-actions"><button class="primary-btn" style="width:100%;text-align:center" data-action="attach-session-payment" data-attach-session="${session.id}">+ Add existing payment</button><button class="ghost-btn" data-action="close-session">Close session</button></div>` : `<div class="footer-note">This session is closed. Payments can no longer be attached to it.</div>`}</section>`
...
function renderStudents() {
  return `<div class="view"><section class="panel glass students-panel"><div class="panel-header students-panel-header"><div class="students-panel-title"><h3>Classmates</h3><span class="muted">Manually managed student list</span></div><div class="students-panel-actions">${button("+ Add Student", "primary-btn", 'data-action="add-student"')}</div></div><div class="search-row students-search-row"><input class="input search-input" id="studentSearch" placeholder="Search student name..."> <select class="select students-filter" id="studentGenderFilter"><option value="all">All genders</option><option value="Male">Boys</option><option value="Female">Girls</option></select> <select class="select students-filter" id="studentStatusFilter"><option value="all">All statuses</option><option value="Active">Participating</option><option value="Inactive">Not participating</option></select></div><div id="studentsTable"></div></section></div>`;
}
