...
function renderStudents() {
  ...
  return `<div class="view"><section class="panel glass students-panel"><div class="panel-header"><div><h3>Classmates</h3><span class="muted">Manually managed student list</span></div>${button("+ Add Student", "primary-btn", 'data-action="add-student"')}</div><div class="gender-tabs" id="studentGenderTabs">${tab("all", "All", total)}${tab("Male", "Boys", boys)}${tab("Female", "Girls", girls)}</div><div class="search-row"><input class="input search-input" id="studentSearch" placeholder="Search student..."> <select class="select compact-filter" id="studentPayFilter" aria-label="Payment status"><option value="all">Payment status</option><option value="due">Due today</option><option value="outstanding">Outstanding</option><option value="partial">Partial</option><option value="paid">Paid</option></select> <select class="select compact-filter" id="studentStatusFilter" aria-label="Participation"><option value="all">Participation</option><option value="Active">Participating</option><option value="Inactive">Not participating</option></select></div><div id="studentsTable"></div></section></div>`;
}
