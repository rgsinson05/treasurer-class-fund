...
function isNoClassDate(key){return Array.isArray(state.settings.noClassDates)&&state.settings.noClassDates.some(x=>(typeof x==='string'?x:x?.date)===key)}
function noClassDatesList(){return (Array.isArray(state.settings.noClassDates)?state.settings.noClassDates:[]).map(x=>typeof x==='string'?{date:x,label:''}:{date:x.date,label:x.label||''}).filter(x=>/^\d{4}-\d{2}-\d{2}$/.test(x.date)).sort((a,b)=>a.date.localeCompare(b.date))}
async function addNoClassDate(){
  const dateInput=$('#noClassDateInput');const labelInput=$('#noClassLabelInput');
  const date=dateInput?.value;if(!date)return showToast('Pick a date first.','error');
  const label=(labelInput?.value||'').trim();
  const current=noClassDatesList().filter(x=>x.date!==date);
  current.push({date,label});
  await saveSetting('noClassDates',current.sort((a,b)=>a.date.localeCompare(b.date)));
  await log('Added no-class date',`${dateOnly(dateFromKey(date))}${label?` — ${label}`:''} marked as no class.`);
  await refresh();render();showToast('No-class date added');
}
async function removeNoClassDate(date){
  const current=noClassDatesList().filter(x=>x.date!==date);
  await saveSetting('noClassDates',current);
  await log('Removed no-class date',`${dateOnly(dateFromKey(date))} is no longer marked as no class.`);
  await refresh();render();showToast('No-class date removed');
}
function noClassDatesSection(){
  const items=noClassDatesList();
  const rows=items.length?items.map(x=>`<div class="noclass-row"><div class="noclass-row-info"><strong>${dateOnly(dateFromKey(x.date))}</strong>${x.label?`<span>${esc(x.label)}</span>`:''}</div>${button('Remove','small-btn danger',`data-remove-noclass="${x.date}"`)}</div>`).join(''):'<div class="empty">No holidays added yet.</div>';
...
async function startCollectionSessionForDate(dateKey){
  if(getCollectionSession(dateKey))return showToast('A collection session already exists for that date.','error');
  const session={id:uid('sess'),date:dateKey,status:'open',startedAt:new Date().toISOString(),paymentIds:[],cashCounted:0,difference:0,note:''};state.collectionSessions.unshift(session);await persistCollectionSessions();await log('Started collection session',`Collection session started for ${dateOnly(dateFromKey(dateKey))}.`);render();showToast('Collection session started');
}
...
document.addEventListener('click',async e=>{
  ...
  if(!dataReady && !e.target.closest('#menuBtn') && !e.target.closest('#lockBtn')){
    ...
    if(action||e.target.closest('[data-quick-record]')||e.target.closest('[data-payment-history]')||e.target.closest('[data-edit-student]')||e.target.closest('[data-delete-student]')||e.target.closest('[data-record-student]')||e.target.closest('[data-edit-contribution]')||e.target.closest('[data-delete-contribution]')||e.target.closest('[data-edit-expense]')||e.target.closest('[data-delete-expense]')||e.target.closest('[data-remove-noclass]')){showToast('Local data is still loading. Please wait a moment.','error');return;}
  }
  ...
  if(action==='attach-session-payment'){const abtn=e.target.closest('[data-attach-session]');return attachPaymentToSession(abtn?.dataset.attachSession);}
  ...
  if(action==='save-settings'){const amount=Number($('#settingAmount').value);if(!Number.isFinite(amount)||amount<=0)return showToast('Daily contribution amount must be greater than zero.','error');await saveSetting('className',$('#settingClass').value.trim()||'BSCS2C');await saveSetting('eventName',$('#settingEvent').value.trim()||'Christmas Party');await saveSetting('contributionAmount',amount);await log('Updated settings','Class, event, or daily contribution amount changed.');await refresh();render();return showToast('Settings saved')}
  ...
  const rnc=e.target.closest('[data-remove-noclass]');if(rnc)return removeNoClassDate(rnc.dataset.removeNoclass);
